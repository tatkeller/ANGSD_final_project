...
DGE.results.sorted <- DGE.results[order(DGE.results$pvalue),]
```
```{r}
table(DGE.results$pvalue < 0.05)
head(DGE.results.sorted)
plotCounts(DESeq.ds, gene="HSPA4L", normalized = TRUE)
plotCounts(DESeq.ds, gene="TPMT", normalized = TRUE)
plotCounts(DESeq.ds, gene="FRK", normalized = TRUE)
plotCounts(DESeq.ds, gene="FRK", normalized = TRUE)
plotCounts(DESeq.ds, gene="RAPGEF5", normalized = TRUE)
plotCounts(DESeq.ds, gene="ERRFI1", normalized = TRUE)
```

```{r}
DGEgenes <- rownames(subset(DGE.results.sorted, pvalue < 0.05))
...
vp1 <- EnhancedVolcano(DGE.results, lab = rownames(DGE.results), x = 'log2FoldChange', y = 'padj', pCutoff = 0.05, title = "Fatal / Survivor")
print(vp1)
```